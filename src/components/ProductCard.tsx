...
export function ProductCard({ title, price, oldPrice, discount, benefits, videoUrl, productName, redirectUrl, isCompact = false }: ProductCardProps) {
  ...
  const handleGeneratePix = async () => {
    setLoading(true);
    try {
      const response = await createPix(price);
  ...
  if (showPayment) {
    if (isCompact) {
      return (
        <div className="bg-[#2a2a2a]/90 rounded-2xl border border-amber-500/10 p-4 flex flex-col gap-3">
          <h3 className="text-xl font-bold text-center text-amber-500">Pagamento via PIX</h3>

          <div className="bg-[#1f1f1f] rounded-xl p-3 border border-amber-500/20">
            <p className="text-base font-semibold text-white">💰 Valor: R$ {price.toFixed(2).replace('.', ',')}</p>
          </div>

          <div className="bg-[#1f1f1f] rounded-xl p-3 border border-amber-500/20">
            <h4 className="text-white font-semibold mb-2 text-sm">Como pagar:</h4>
            <ol className="text-gray-300 text-xs space-y-1">
              <li><span className="text-amber-500 font-bold">1.</span> Copie o código PIX</li>
              ...
              <li><span className="text-amber-500 font-bold">4.</span> Cole e confirme</li>
              <li><span className="text-amber-500 font-bold">5.</span> Clique em "Verificar"</li>
            </ol>
          </div>
          ...
          <button
            ...
            className="w-full py-2 rounded-lg bg-blue-600 hover:bg-blue-700 transition font-bold text-white text-sm flex items-center justify-center gap-1"
          >
            📋 Copiar Código
          </button>
          ...
          <button
            ...
            disabled={verifying}
            className="w-full py-2 rounded-lg bg-amber-600 hover:bg-amber-700 transition font-bold text-white text-sm flex items-center justify-center gap-1 disabled:opacity-50"
          >
            {verifying ? 'Verificando...' : '✔️ Verificar'}
          </button>

          <p className="text-center text-gray-400 text-xs">Após pagar, clique em "Verificar"</p>
        </div>
      );
    }
  ...
  if (isCompact) {
    return (
      <div className="bg-[#2a2a2a]/90 rounded-2xl border border-amber-500/10 overflow-hidden">
        <div className="px-4 py-3 flex flex-col gap-2">
          <h3 className="text-lg font-bold text-center text-amber-500">{title}</h3>

          <div className="bg-red-950/50 border border-red-800/50 rounded-lg p-2 flex items-center justify-center gap-2">
            <Clock size={16} className="text-red-400" />
            <span className="text-white font-semibold text-sm">Apenas R$ {price.toFixed(2).replace('.', ',')} HOJE</span>
          </div>
          ...
          <div className="space-y-1">
            {benefits.map((benefit, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <span className="text-gray-200 text-xs">{benefit.text}</span>
              </div>
            ))}
          </div>

          <div className="bg-[#1f1f1f] rounded-lg p-3 border border-amber-500/20 flex flex-col items-center gap-1">
            <div className="flex items-center gap-2">
              <span className="text-gray-400 line-through text-xs">R$ {oldPrice.toFixed(2).replace('.', ',')}</span>
              <span className="text-amber-500 text-xl font-bold">R$ {price.toFixed(2).replace('.', ',')}</span>
            </div>
            <span className="text-green-400 font-semibold text-xs">{discount} • Acesso Vitalício</span>
          </div>
          ...
          <p className="text-center text-gray-400 text-xs">Pagamento via PIX • Liberação Imediata</p>
        </div>
      </div>
    );
  }
